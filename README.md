...
The following diagram illustrates the TamTam architecture: local links, their orchestrators acting as transfer stations, and the shared interchange (roundabout).

```mermaid
graph TB

  subgraph Link_A[Link A – Local Domain]
    A1[Node]
    A2[Node]
    A3[Service Port]
    A1 --> A2 --> A3
  ...
  subgraph OrchA[Orchestrator A]
    OA[Dispatcher & Gateway]
  end

  subgraph Interchange[Interchange / Roundabout]
    R1[Orchestrator A Presence]
    R2[Orchestrator B Presence]
    R3[Orchestrator C Presence]
  end

  subgraph OrchB[Orchestrator B]
    OB[Dispatcher & Gateway]
  end

  subgraph Link_B[Link B – Different Runtime]
    B1[Node]
    B2[Service Port]
    B1 --> B2
  end
...
**Legend:**
- Each *Link* is a closed local domain.
- The *Orchestrator* is the only gateway between a link and the interchange.
- The *Interchange* connects heterogeneous orchestrators using ring-forwarding and optional direct routing.
- Implementations may differ (C, C#, CLI, embedded), but the protocol and roles remain the same.
